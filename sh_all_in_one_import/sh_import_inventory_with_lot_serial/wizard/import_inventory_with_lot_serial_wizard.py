...
class ImportInventoryWithLotSerialWizard(models.TransientModel):
    ...
    def import_inventory_with_lot_serial_apply(self):
        '''............perform import inventory with lot serial...........'''
        stock_production_lot_obj = self.env['stock.lot']
                    ...
                    for row in values:
                        try:
                            ...
                            if row[0] not in (None, ""):
                                ...
                                if search_product and search_product.type == 'product' and search_product.tracking in ['serial', 'lot'] and row[3] != '':
                                    quant_id = self.env['stock.quant'].search([('location_id', '=', self.location_id.id), (
                                        'company_id', '=', self.env.company.id), ('product_id', '=', search_product.id), ('lot_id', '=', row[3])], limit=1)
                                    quant_vals = {}
                                    if quant_id:
                                        quant_vals.update({
                                        ...
                                        if row[1] not in (None, ""):
                                            if search_product.tracking == 'serial':
                                                if str(row[1]) != str(1):
                                                    if str(row[1]) == str(0):
                                                        quant_vals.update(
                                                        ...
                                                        skipped_line_no[str(
                                                            counter)] = " -serial number must be unique for each quantity here more than one."
                                                        counter = counter + 1
                                                        continue
                                                else:
                                                    quant_vals.update(
